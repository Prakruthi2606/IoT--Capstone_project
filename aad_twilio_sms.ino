...
const char* twilioNumber = "+16282887985";  // Twilio-provided WhatsApp number
const char* recipientNumber = "+917676607253";  // Guardian’s WhatsApp number
// Wi-Fi credentials
...
void sendSMSAlert(float lat, float lon) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // SMS Content with Google Maps link
    String smsMessage = "🚨 Accident detected! Location: https://maps.google.com/?q=" + 
                         String(lat, 6) + "," + String(lon, 6);

    // Twilio API URL
    String twilioUrl = "https://api.twilio.com/2010-04-01/Accounts/" + String(twilioSID) + "/Messages.json";
    
    // Base64 Encode SID:AuthToken for HTTP Basic Authentication
    ...
    String postData = "To=" + String(recipientNumber) +   // Recipient's phone number
                      "&From=" + String(twilioNumber) +   // Twilio Number
                      "&Body=" + smsMessage;              // Message Content
